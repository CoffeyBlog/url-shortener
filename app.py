...
# if the url entered by the user is already in the .json file ... return the user to home
        if request.form['code'] in urls.keys():
            flash('That short name has already been taken, please select another name')                                 # if the url entered by the user is already in the .json file ... flash an h2 tag via jinja that says URL has been taken
            return redirect(url_for('home'))



# oes through form to see if there is something called url
        if 'url' in request.form.keys():
            urls[request.form['code']] = {'url':request.form['url']}
        else:
            f = request.files['file']                                                                                   # is short for file
            full_name = request.form['code'] + secure_filename(f.filename)                                              # wekzeug is a security library created by the flask team
            f.save('/Users/lucascoffey/Desktop/url-shortener/' + full_name)
